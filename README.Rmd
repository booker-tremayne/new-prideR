...
It can accept:
  ...
  * __Various Sorting methods__: which can specify how many elements per page, which page, and whether all keywords need to be matched to return a project or any keyword.
...
Note that this returns a list of FileDetailList objects. Also note that this is case sensitive.
...
We wish to obtain the projects that can be analyzed with *Cardinal*. For this, we need projects that contain ".ibd" and ."imzml" files. First, we should obtain all projects that contain the word "imaging" or "msi". This is necessary since the file searching method is slow when searching across several projects.
